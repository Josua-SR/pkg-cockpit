(function(e) {
    ...
    function x() {
        function t(t, n) {
            ...
            try {
                ...
                b(a(d("The web browser configuration prevents Cockpit from running (inaccessible $0)"), t));
                throw i;
            }
            if (o === undefined) {
                if (window.console) e.warn(a(d("This web browser is too old to run Cockpit (missing $0)"), t));
                h("login").style.display = "none";
                h("login-details").style.display = "none";
        ...
        return ("MozWebSocket" in window || t("WebSocket", window)) && t("XMLHttpRequest", window) && t("sessionStorage", window) && t("JSON", window) && t("defineProperty", Object) && t("console", window) && t("pushState", window.history) && t("textContent", document) && t("CSS", window) && t("supports", window.CSS);
    }
    function S(e) {
    ...
    function O() {
        ...
        if (!x()) return;
        h("show-other-login-options").addEventListener("click", C);
        h("show-other-login-options").addEventListener("keypress", C);
    ...
    function E() {
        ...
        v = r.URL;
        if (r.URL.indexOf("?") > -1) v += "&"; else v += "?";
        v += "redirect_uri=" + encodeURIComponent(e[0]);
    }
    function L() {
        var e = document.createElement("a");
        if (!r.URL) return b(d("Cockpit authentication is configured incorrectly."));
    ...
    function z() {
        R(null);
        var e, n = S(h("login-user-input").value);
        if (n === "") {
            R(d("User name cannot be empty"));
        } else if (U() && h("server-field").value === "") {
            R(d("Please specify the host to connect to"));
        } else {
    ...
    function q() {
        ...
        N(d("Log in with your server user account."));
        ...
        i.onreadystatechange = function() {
            if (i.readyState != 4) {
                ...
                if (s && s.toLowerCase().indexOf("x-conversation") === 0) {
                    r = $(s, i.responseText);
                    if (r) J(r); else b(d("Internal Error: Invalid challenge header"));
                } else {
                    if (window.console) e.log(i.statusText);
                    if (i.statusText.indexOf("authentication-not-supported") > -1) {
                        var n = S(h("login-user-input").value);
                        b(a(d("The server refused to authenticate '$0' using password authentication, and no other supported authentication methods are available."), n));
                    } else if (i.statusText.indexOf("terminated") > -1) {
                        R(d("Authentication Failed: Server closed connection"));
                    } else if (i.statusText.indexOf("no-host") > -1) {
                        H(d("Unable to connect to that address"));
                    } else if (i.statusText.indexOf("unknown-hostkey") > -1) {
                        H(d("Refusing to connect. Hostkey is unknown"));
                    } else if (i.statusText.indexOf("unknown-host") > -1) {
                        H(d("Refusing to connect. Host is unknown"));
                    } else if (i.statusText.indexOf("invalid-hostkey") > -1) {
                        H(d("Refusing to connect. Hostkey does not match"));
                    } else if (o) {
                        R(d("Authentication failed"));
                    } else {
                        R(d("Wrong user name or password"));
                    }
                }
            } else if (i.status == 403) {
                R(decodeURIComponent(i.statusText) || d("Permission denied"));
            } else if (i.statusText) {
                b(decodeURIComponent(i.statusText));
            } else {
                b(a(d("$0 error"), i.status));
            }
            h("login-button").removeAttribute("disabled");
        };
        i.send();
