!function(t) {
	 ...
	 */
        !function(r, o) {
            ...
            function a(e, i) {
                ...
                i = i || t.tags, e = e || "", "string" == typeof i && (i = i.split(v));
                for (var a, u, f, d, p, m, g = o(i), y = new h(e), C = [], M = [], S = [], T = !1, E = !1; !y.eos(); ) {
                    if (a = y.pos, f = y.scanUntil(g[0])) for (var _ = 0, q = f.length; _ < q; ++_) d = f.charAt(_), 
                    s(d) ? S.push(M.length) : E = !0, M.push([ "text", d, a, a + 1 ]), a += 1, "\n" === d && r();
                    if (!y.scan(g[0])) break;
                    if (T = !0, u = y.scan(k) || "name", y.scan(b), "=" === u ? (f = y.scanUntil(w), 
                    y.scan(w), y.scanUntil(g[1])) : "{" === u ? (f = y.scanUntil(new RegExp("\\s*" + n("}" + i[1]))), 
                    y.scan(x), y.scanUntil(g[1]), u = "&") : f = y.scanUntil(g[1]), !y.scan(g[1])) throw new Error("Unclosed tag at " + y.pos);
                    if (p = [ u, f, a, y.pos ], M.push(p), "#" === u || "^" === u) C.push(p); else if ("/" === u) {
                        if (m = C.pop(), !m) throw new Error('Unopened section "' + f + '" at ' + a);
                        if (m[1] !== f) throw new Error('Unclosed section "' + m[1] + '" at ' + a);
                    } else "name" === u || "{" === u || "&" === u ? E = !0 : "=" === u && (g = o(i = f.split(v)));
                }
                if (m = C.pop()) throw new Error('Unclosed section "' + m[1] + '" at ' + y.pos);
                return l(c(M));
            }
            function c(t) {
            ...
            var d = RegExp.prototype.test, p = /\S/, m = Object.prototype.toString, g = Array.isArray || function(t) {
            ...
            h.prototype.eos = function() {
                ...
                a[0]) {
                  case "#":
                    ...
                    if (g(c)) for (var d = 0, p = c.length; d < p; ++d) l += this.renderTokens(a[4], s.push(c[d]), n, r); else if ("object" == typeof c || "string" == typeof c) l += this.renderTokens(a[4], s.push(c), n, r); else if (i(c)) {
                  ...
                  case "^":
                    c = s.lookup(a[1]), (!c || g(c) && 0 === c.length) && (l += this.renderTokens(a[4], s, n, r));
                    break;
    ...
    15: function(t, e, s) {
        ...
        !function() {
            ...
            l.resource_debug = function() {
            ...
            }, l.render_container = function(t, e, s, r, h, u) {
                var f = i(document.getElementById(s + r));
                if (!h) return f.remove(), void (e.find("table > tbody > tr").length || e.find("button.enable-danger").toggle(!1));
                var d, p, m, g, y, b;
                h.State && h.State.Running ? (d = l.format_cpu_usage(h.CpuUsage), p = h.MemoryUsage, 
                m = h.MemoryLimit, y = l.format_memory_and_limit(p, m), g = !0, b = {
                    color: "inherit"
                }) : (d = "", g = !1, y = c("Stopped"), b = {
                    color: "grey",
                    "text-align": "right"
                ...
                var M = f.children("td");
                i(M[0]).text(l.render_container_name(h.Name));
                var S = h.Image;
                h.ImageID && S == h.ImageID && (S = o.truncate_id(S)), i(M[1]).text(S), i(M[2]).text(l.render_container_cmdline(h)), 
                i(M[3]).text(d), l.update_memory_bar(i(M[4]).children("div").toggle(g), p, m), i(M[5]).css(b).text(y);
                var T = r in t.waiting;
                i(M[6]).children("div.spinner").toggle(T), i(M[6]).children("button.btn-delete").toggle(!T).toggleClass("disabled", h.State.Running);
                var E = T || h.State.Running ? "You can only delete<br/> stopped containers" : "Delete immediately";
                i(M[6]).children("button.btn-delete").tooltip("destroy").attr("title", E).tooltip({
                    html: !0
                }), i(M[7]).children("div.spinner").toggle(T), i(M[7]).children("button.fa-play").toggle(!T && !h.State.Running), 
                i(M[7]).children("button.fa-stop").toggle(!T && h.State.Running), i(M[6]).toggle(u), 
                i(M[7]).toggle(!u), f.toggleClass("unimportant", !h.State.Running), v && l.insert_table_sorted(e.find("table"), f), 
                a.update();
	 ...
	 */
            !function(t) {
                ...
                function b() {
                    $.blocking = !0, $.testTimeout && B.setTimeout && (I($.timeout), $.timeout = O(function() {
                ...
                function v() {
                    Q = !0, B.setTimeout ? O(function() {
                        $.current && $.current.semaphore > 0 || ($.timeout && I($.timeout), g());
                    }, 13) : g();
                }
                function w() {
                    var t, e;
                ...
                }, O = t.setTimeout, I = t.clearTimeout, j = t.window, B = {
                ...
                if (q) {
                    ...
                    QUnit.config.urlConfig.push({
                        ...
                        I && "complete" === L.readyState ? QUnit.load() : s(window, "load", QUnit.load), 
                        QUnit.diff = function() {
                            function t() {}
                            var s = -1, i = 1, n = 0;
