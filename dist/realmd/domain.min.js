            ...
            function a(e, a, r, m) {
                ...
                function v(a) {
                    ...
                    t.always(function() {
                        ...
                        t.reject(o));
                        var d;
                        arguments[0][1] && (i = arguments[0][1]), l = i[0], l ? ($ = e.proxy(p, l), A($).on("changed", b), 
                        O = e.proxy(c, l), r = e.proxy(u, l), A(r).on("changed", b), r.wait(function() {
                            t.resolve(r);
                        })) : (a && (d = n.format(s("Domain $0 could not be contacted"), a), A(".realms-op-address-error").show().attr("title", d)), 
                        r = null, $ = null, O = null, t.reject(new Error(d))), b();
                    }), D = t.promise(), P = a, D;
                }
                function h() {
                    if ("join" != a) {
                    ...
                    return A(".realms-op-address").val() === P && D ? D : v();
                }
                function g(e, a) {
                    var t = null;
                ...
                function b() {
                    ...
                    if (r && $ && !$.valid ? (t = n.format(s("Domain $0 is not supported"), r.Name), 
                    ...
                    N ? m.attr("disabled", "disabled") : m.removeAttr("disabled"), "join" == a) {
                        ...
                        var l = "";
                        $ && $.SuggestedAdministrator && (l = n.format(s('e.g. "$0"'), $.SuggestedAdministrator)), 
                        A(".realms-op-admin")[0].placeholder = l;
                        var i = A(".realms-op-auth .dropdown-menu"), d = $ && $.SupportedJoinCredentials || [];
                ...
                function y() {
                    ...
                    if ("password/administrator" !== E) return console.log("Installing kerberos keytab not supported for auth mode", E), 
                    n.resolve();
                    var e = A(".realms-op-admin").val(), a = A(".realms-op-admin-password").val(), t = "set -eu; [ $(id -u) = 0 ] || exit 0; ";
                    t += "type ipa >/dev/null 2>&1 || exit 0; ", t += "kinit " + e + "@" + O.RealmName + " || exit 0; ", 
                    t += 'service="HTTP/$(hostname -f)@' + O.RealmName + '"; ipa service-show "$service" || ipa service-add --ok-as-delegate=true --force "$service"; ', 
                    t += "mkdir -p /etc/cockpit; ", t += 'klist -k /etc/cockpit/krb5.keytab | grep -qF "$service" || ipa-getkeytab -p HTTP/$(hostname -f) -k /etc/cockpit/krb5.keytab; ';
                    var r = n.script(t, [], {
                        superuser: "require",
                        err: "message",
                ...
                function k() {
                    if ("localhost" !== n.transport.host) return n.resolve();
                    var a = n.defer();
                    return O = e.proxy(c, r.path), O.wait().done(function() {
                        n.script('[ ! -e /etc/cockpit/krb5.keytab ] || ipa-rmkeytab -k /etc/cockpit/krb5.keytab -p "HTTP/$(hostname -f)@' + O.RealmName + '"', [], {
                            superuser: "require",
                            err: "message"
                ...
                function x() {
                    var t = "cockpit-" + q;
                    q += 1, j(t), A(".realms-op-error").hide(), h().fail(function() {
                        j(null);
                    }).done(function(r) {
                        var o = {
                        ...
                        $.valid ? l = $.call("Join", [ w(), o ]).then(y) : (j(null), A(".realms-op-message").empty().text(s("Joining this domain is not supported")), 
                        A(".realms-op-error").show())) : "leave" == a && (l = k().then(function() {
                            r.Deconfigure(o);
                        })), l ? void l.fail(function(e) {
                            j(null), "org.freedesktop.realmd.Error.Cancelled" == e.name ? A(T).modal("hide") : (console.log("Failed to " + a + " domain: " + r.Name + ": " + e), 
                ...
                function C() {
                    return !!N && (e.call(l, i, "Cancel", [ N ]), j(null), !0);
                }
                var T = o.parseHTML(t(6))[0], A = function(e, a) {
                    return new o.fn.init(e, a || T);
                };
                A.fn = A.prototype = o.fn, o.extend(A, o);
                var N = null, D = null, P = null, $ = null, O = null;
                A(".realms-op-cancel").on("click", function() {
                    C() || A(T).modal("hide");
                }), A(T).on("hide.bs.modal", function() {
                    C();
                }), A(".realms-op-apply").on("click", x), A(".realms-op-field").on("keydown", function(e) {
                    13 == e.which && x();
                }), A(T).on("click", ".realms-op-more-diagnostics", function() {
                    A(".realms-op-error").hide(), A(".realms-op-diagnostics").show();
                });
                var S = null;
                A(".realms-op-address").on("keyup change", function() {
                    A(".realms-op-address").val() != P && (A(".realms-op-address-error").hide(), window.clearTimeout(S), 
                    S = window.setTimeout(v, 1e3));
                });
                var E = null;
                ...
                var J, R, L;
                "join" == a ? (J = s("page-title", s("Join a Domain")), R = s("Join"), A(".realms-op-join-only-row").show(), 
                A(".realms-op-leave-only-row").hide(), v("")) : (J = s("page-title", s("Leave Domain")), 
                R = s("Leave"), L = s("Are you sure you want to leave this domain?"), r && r.Name && (L = n.format(s("Are you sure you want to leave the '$0' domain?"), r.Name)), 
                L = n.format(s("$0 Only users with local credentials will be able to log into this machine. This may also effect other services as DNS resolution settings and the list of trusted CAs may change."), L), 
                A(".realms-op-leave-only-row").text(L), A(".realms-op-join-only-row").hide(), A(".realms-op-leave-only-row").show()), 
                A(".realms-op-title").text(J), A(".realms-op-apply").text(R), A(".realms-op-field").val("");
                var q = 1;
                return b(), T;
            }
            function r() {
